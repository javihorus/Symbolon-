(() => {
  ...
  async function init(cb){ callback=cb; if(!isConfigured()){emit();return;} client=window.supabase.createClient(cfg.url,cfg.anonKey); const {data}=await client.auth.getSession(); user=data?.session?.user||null; emit(); client.auth.onAuthStateChange((_e,s)=>{user=s?.user||null;emit();}); }
  async function signIn(email){ if(!client) throw new Error('Supabase no está configurado'); const redirectTo=location.origin+location.pathname; const {error}=await client.auth.signInWithOtp({email,options:{emailRedirectTo:redirectTo}}); if(error) throw error; }
